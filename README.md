...
MomentMap remembers why it mattered — how it felt, what happened there, and the
...
- **Photo Memory** — turn a batch of photos into one or more Memories: MomentMap groups
...
- **Share** — send a Place or a whole Collection to another MomentMap user via the
...
- **CloudKit** (private database) backs the SwiftData store for cross-device sync —
  see `MomentMap/App/MomentMapApp.swift` and `DATA_MODEL.md` §23.
- **MapKit** for place search/resolution (`MapSearchService`,
  `NearbyPlaceSearchService`) — MomentMap never invents place identity; every Place
...
- **Share Extension** (`MomentMapShareExtension`) lets other apps hand a URL, text,
  or a MomentMap-exported Place/Collection file to MomentMap.
...
```bash
xcodegen generate
xcodebuild build -project MomentMap.xcodeproj -scheme MomentMap \
...
```bash
xcodebuild test -project MomentMap.xcodeproj -scheme MomentMap \
  -destination 'platform=iOS Simulator,name=iPhone 17'
```
...
| Doc | Covers |
| --- | --- |
| `MANIFESTO.md` | Why MomentMap exists, what it chooses not to be |
